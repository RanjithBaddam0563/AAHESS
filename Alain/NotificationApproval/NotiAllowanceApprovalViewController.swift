//
//  NotiAllowanceApprovalViewController.swift
//  Alain
//
...
class NotiAllowanceApprovalViewController: UIViewController,UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    func fetchEligibityBalance()
    {
            if Connectivity.isConnectedToInternet {
                ...
                let par = MyStrings().AllowanceEligibityBalance + self.AllowanceTypeId + "/" + String(userId)
                let params = par
                print(params)
    ...
    func ViewTimefetchEligibityBalance()
    {
            if Connectivity.isConnectedToInternet {
                ...
                let par = MyStrings().GetAllowanceEligibityBalance + self.SendAllowanceTypeId
                let params = par
                print(params)
                let token =  UserDefaults.standard.string(forKey: "Token")!
                var request = URLRequest(url: URL(string: params)!,timeoutInterval: Double.infinity)
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        self.paticularsTF.text = AllowanceTypeDetails[row].name
    ...
    func getallowanceApplicationId()
    {
            if Connectivity.isConnectedToInternet {
                ...
                let par = MyStrings().AllowanceIDInfo + self.ResponseID
                let params = par
                print(params)
                let token =  UserDefaults.standard.string(forKey: "Token")!
                var request = URLRequest(url: URL(string: params)!,timeoutInterval: Double.infinity)
               ...
               let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                DispatchQueue.main.async {
                    self.allowanceApplicationId = String(json["allowanceApplicationId"].intValue)
    ...
    func getallowanceDetails()
    {
            if Connectivity.isConnectedToInternet {
                ...
                let par = MyStrings().AllowanceDetailsInfo + self.allowanceApplicationId
                let params = par
                print(params)
                let token =  UserDefaults.standard.string(forKey: "Token")!
                var request = URLRequest(url: URL(string: params)!,timeoutInterval: Double.infinity)
               ...
               let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                DispatchQueue.main.async {
                    self.allowanceApplicationId1 = String(json["allowanceApplicationId"].intValue)
                    self.SendAllowanceTypeId = String(json["allowanceTypeId"].intValue)
                    ...
                    {
                        self.paticularsTF.text = "Housing Allowance"
                    }else{
    ...
    func ApprovalHistory()
    {
                ...
                if Connectivity.isConnectedToInternet {
                    ...
                    let par = MyStrings().NotAllowanceApprovalDetailsInfo
    ...
    @IBAction func ClickOnSubmit(_ sender: Any)
    {
        ...
        let parameters = "{\n    \"AllowanceTaskId\": \(ResponseID),\"Outcome\":  \"\(self.ApprovalTF.text!)\",\"Comment\":  \"\(self.CommentsTF.text!)\"\n}"
        let postData = parameters.data(using: .utf8)

        var request = URLRequest(url: URL(string: MyStrings().PostAllowanceApprovalDetails + self.ResponseID)!,timeoutInterval: Double.infinity)
        let token =  UserDefaults.standard.string(forKey: "Token")!
          ...
          print(String(data: data, encoding: .utf8)!)
            print("response = \(String(describing: response))")
                       if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode != 204 {           // check for http errors
                           ...
                           DispatchQueue.main.async {
                             self.AskConfirmation(title: "My Alert", message: "Allowance Request Approval Details Updated Successfully") { (result) in
                             if result {
                             self.navigationController?.popViewController(animated: true)
                             } else {
...
extension NotiAllowanceApprovalViewController: UITableViewDataSource,UITableViewDelegate {
...
func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
{
    ...
    let cell = self.TBV.dequeueReusableCell(withIdentifier: "ApprovalTableViewCell", for: indexPath)as! ApprovalTableViewCell
