//
//  formalityViewViewController.swift
//  Alain
//
...
class formalityViewViewController: UIViewController {
    ...
    @IBOutlet weak var detailsReqLbl: UILabel!
    ...
    var formalityViewListDetails : [formalityViewListModel] = []
    var formalityApprovalListDetails = [[String: Any]]()
//    var employeeAccountingInfoDetails : [[String: Any]] = []
    ...
    //            self.DMSReqViewListDetails.removeAll()
                if Connectivity.isConnectedToInternet {
                    ...
                    let par = MyStrings().FormalityViewListInfo
                   ...
                   let task = URLSession.shared.dataTask(with: request) { data, response, error in
                    ...
                    print(json)
                    DispatchQueue.main.async {
                        self.dict2 =  json["employeeAccountingInfo"].dictionaryValue
                        
                        if let adUserName = self.dict2["salary"]as? Any
                        {
                            let name = dump(self.toString(self.dict2["salary"]))
                            self.salaryLbl.text = name
                        }else{
                            self.salaryLbl.text = ""
                        }
                        ...
                        self.paticularsLbl.text = self.formalityType
                       let detailsReq = json["detailsRequired"].boolValue
                        if detailsReq == true
                        {
                            self.detailsReqLbl.text = "Yes"
                        }else{
                            self.detailsReqLbl.text = "No"
                        }
                        self.purposeLbl.text = json["purpose"].stringValue
                        
//                        self.salaryLbl.text = json["purpose"].stringValue
      ...
      func ApprovalHistory()
        {
                    ...
                    if Connectivity.isConnectedToInternet {
                        ...
                        let par = MyStrings().formalityApprovalListInfo
                        let params = par + "/" + self.formalityId
                        print(params)
                        let token =  UserDefaults.standard.string(forKey: "Token")!
                        var request = URLRequest(url: URL(string: params)!,timeoutInterval: Double.infinity)
                       ...
                       for (index,subJson):(String, JSON) in json {
                          print(index)
                          print(subJson)
                          let details1 = formalityViewListModel.init(json: subJson)
    ...
    extension formalityViewViewController: UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        let cell = self.TBV.dequeueReusableCell(withIdentifier: "MyDMSReqViewTableViewCell6", for: indexPath)as! MyDMSReqViewTableViewCell
