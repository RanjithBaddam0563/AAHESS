//
//  ReimbursmentReqViewViewController.swift
//  Alain
//
...
class ReimbursmentReqViewViewController: UIViewController {
    var ReimbursmentId : String = ""
    var ReimbursmentType : String = ""
    ...
    var ReimbursmentListDetails : [ReimbursmentViewListModel] = []
      var ReimbursmentApprovalListDetails = [[String: Any]]()
    override func viewDidLoad() {
        super.viewDidLoad()
        ReimbursmentViewDetails()
        ApprovalHistory()
    ...
    //            self.DMSReqViewListDetails.removeAll()
                if Connectivity.isConnectedToInternet {
                    ...
                    let par = MyStrings().ReimbursmentViewListInfo
                    let params = par + "/" + self.ReimbursmentId
                    print(params)
                   ...
                   let task = URLSession.shared.dataTask(with: request) { data, response, error in
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.paticularsLbl.text = self.ReimbursmentType
      ...
      func ApprovalHistory()
        {
                    self.ReimbursmentListDetails.removeAll()
                    if Connectivity.isConnectedToInternet {
                        DispatchQueue.main.async {
                            MBProgressHUD.showAdded(to: self.view, animated: true)
                        }

                        let par = MyStrings().ReimbursmentApprovalListInfo
                        let params = par + "/" + self.ReimbursmentId
                        print(params)
                        let token =  UserDefaults.standard.string(forKey: "Token")!
                        var request = URLRequest(url: URL(string: params)!,timeoutInterval: Double.infinity)
                       ...
                       for (index,subJson):(String, JSON) in json {
                          print(index)
                          print(subJson)
                          let details1 = ReimbursmentViewListModel.init(json: subJson)
                          self.ReimbursmentListDetails.append(details1)
                          let approver =  subJson["approver"].dictionaryValue
                        self.ReimbursmentApprovalListDetails.append(approver)
                       }
                        print(self.ReimbursmentApprovalListDetails)
                        DispatchQueue.main.async {
    ...
    extension ReimbursmentReqViewViewController: UITableViewDataSource,UITableViewDelegate {


    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
        if self.ReimbursmentListDetails.count == 0
        {
            return 0
        }else{
            return self.ReimbursmentListDetails.count
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        let cell = self.TBV.dequeueReusableCell(withIdentifier: "MyDMSReqViewTableViewCell1", for: indexPath)as! MyDMSReqViewTableViewCell
        let dict = self.ReimbursmentApprovalListDetails[indexPath.row] as [String : Any]
        print(dict)
        ...
        cell.ApprovalLbl.text = self.ReimbursmentListDetails[indexPath.row].outcome
              let startDate = self.ReimbursmentListDetails[indexPath.row].created
               let checkdate = startDate.fromUTCToLocalDateTime()
                       print(checkdate)
                       cell.DateTimeLbl.text = checkdate
        cell.CommentsLbl.text = self.ReimbursmentListDetails[indexPath.row].comment
