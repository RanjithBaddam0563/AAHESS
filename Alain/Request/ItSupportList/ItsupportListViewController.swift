//
//  ItsupportListViewController.swift
//  Alain
//
...
import Alamofire
import SwiftyJSON
class ItsupportListViewController: UIViewController,UITextFieldDelegate {
    var FormalityDetails : [ItSupportListModel] = []
    var SearchFormalityDetails : [ItSupportListModel] = []
         ...
         func formalityListDetails()
         {
            ...
            self.SearchFormalityDetails.removeAll()
                 if Connectivity.isConnectedToInternet {
                     ...
                     let par = MyStrings().ItSupportListInfo
                     let params = par
                     print(params)
                    ...
                    let task = URLSession.shared.dataTask(with: request) { data, response, error in
                     ...
                     print(json)
                        if json.count == 0
                        {
                            ...
                            let alert = UIAlertController(title: "My alert", message: "IT-Support list empty", preferredStyle: UIAlertController.Style.alert)
                      ...
                      for (index,subJson):(String, JSON) in json {
                          print(index)
                          print(subJson)
                          let details = ItSupportListModel.init(json: subJson)
    ...
    {
     self.formalityId = String(self.SearchFormalityDetails[sender.tag].dmsApplicationId)

         if #available(iOS 13.0, *) {
          let vc = self.storyboard?.instantiateViewController(identifier: "ItSupportReqViewViewController")as! ItSupportReqViewViewController
             vc.formalityId  = self.formalityId
             vc.formalityType = self.SearchFormalityDetails[sender.tag].leaveType
          self.navigationController?.pushViewController(vc, animated: true)

          } else {
              let vc = self.storyboard?.instantiateViewController(withIdentifier: "ItSupportReqViewViewController")as! ItSupportReqViewViewController
             vc.formalityId  = self.formalityId
            vc.formalityType = self.SearchFormalityDetails[sender.tag].leaveType
              self.navigationController?.pushViewController(vc, animated: true)
...
extension ItsupportListViewController: UITableViewDataSource,UITableViewDelegate {
   ...
   func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
   {
    ...
    let cell = self.TBV.dequeueReusableCell(withIdentifier: "ItSupportTableViewCell", for: indexPath)as! ItSupportTableViewCell
       cell.trainingNameLbl.text = self.SearchFormalityDetails[indexPath.row].leaveType
...
extension ItsupportListViewController : UISearchBarDelegate
{
