...
class TrainingListViewController: UIViewController,UITextFieldDelegate {
    var FormalityDetails : [TrainingListModel] = []
    var SearchFormalityDetails : [TrainingListModel] = []
         ...
         func formalityListDetails()
         {
            ...
            self.SearchFormalityDetails.removeAll()
                 if Connectivity.isConnectedToInternet {
                     ...
                     let par = MyStrings().TrainingListInfo
                     let params = par
                     print(params)
                    ...
                    let task = URLSession.shared.dataTask(with: request) { data, response, error in
                     ...
                     print(json)
                        if json.count == 0
                        {
                            ...
                            let alert = UIAlertController(title: "My alert", message: "Training list empty", preferredStyle: UIAlertController.Style.alert)
                      ...
                      for (index,subJson):(String, JSON) in json {
                          print(index)
                          print(subJson)
                          let details = TrainingListModel.init(json: subJson)
    ...
    @objc func ViewBtnAction(sender: UIButton!)
    {
     self.formalityId = String(self.SearchFormalityDetails[sender.tag].id)
         ...
         if #available(iOS 13.0, *) {
          let vc = self.storyboard?.instantiateViewController(identifier: "TrainingReqViewViewController")as! TrainingReqViewViewController
             vc.formalityId  = self.formalityId
             vc.formalityType = self.SearchFormalityDetails[sender.tag].leaveType
          self.navigationController?.pushViewController(vc, animated: true)

          } else {
              let vc = self.storyboard?.instantiateViewController(withIdentifier: "TrainingReqViewViewController")as! TrainingReqViewViewController
             vc.formalityId  = self.formalityId
            vc.formalityType = self.SearchFormalityDetails[sender.tag].leaveType
              self.navigationController?.pushViewController(vc, animated: true)
...
extension TrainingListViewController: UITableViewDataSource,UITableViewDelegate {
   ...
   func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
   {
    ...
    let cell = self.TBV.dequeueReusableCell(withIdentifier: "trainingListTableViewCell", for: indexPath)as! trainingListTableViewCell
...
extension TrainingListViewController : UISearchBarDelegate
{
